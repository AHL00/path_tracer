            ...
            scene
                ...
                .for_each(|node| {
                    if let Some(mesh) = node.mesh() {
                        ...
                        mesh.primitives().collect::<Vec<_>>().par_iter().for_each( |primitive| {
                            let gltf_mat = primitive.material();

                            let material = Material::from_gltf(
                                gltf_mat, scene_resources.clone(), context, &buffers, &gltf_dir,
                            );
                            ...
                            let positions = if let Some(positions) = reader.read_positions() {
                                ...
                                log::warn!(
                                    "Mesh in Node [{:?}] has no positions, skipping primitive",
                                    node.name()
                                );
                                return;
                            };
                            ...
                            let geometry = if let Ok(geometry) = Geometry::create(
                                ident,
                                vertices,
                                indices,
                                ...
                                // TODO: Handle error properly
                                log::warn!(
                                    "Failed to create geometry for Mesh in Node [{:?}], skipping primitive",
                                    node.name()
                                );
                                return;
                            };
                            ...
                            self.world().lock().unwrap().push((geometry, transform, material));
                        });
                    }
                });
        });
