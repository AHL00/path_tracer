...
layout(push_constant) uniform RchitPushConstants {
...
//     const float u32_max = 4294967295.0;
//     float seed_norm = float(push_constants.uniforms.seed) / u32_max;
...
//         scatter_direction = cosine_hemisphere_sample(uv, normal);
...
void main() {
    ...
    if (material.has_base_texture) {
        ...
        albedo = material.base_color.xyz;
    }
    ...
    bool should_scatter = true;
        ...
        vec3 reflected_random = normalize(cosine_hemisphere_sample(
            payload.in_uv, 
            payload.depth, 
        ...
        scatter_direction = mix(reflected, reflected_random, roughness);  

        // Ensure the scattered ray is above the surface
        if (dot(scatter_direction, normal) <= 0.0) {
            // If perturbation sends ray below surface, absorb it (simplest approach)
            should_scatter = false;
            attenuation = vec3(0.0);
        } else {
            // Metallic surfaces reflect with their albedo color
            attenuation = albedo;
        }
    ...
    scatter_origin = world_pos + normal * 0.001;
    ...
    //     if (rand(payload.in_uv, payload.depth, push_constants.uniforms.accumulated_count, push_constants.uniforms.seed) > p) {
    //         payload.done = 1;  // Terminate ray early
    //     } else {
    //         // Compensate for terminated rays
    //         payload.attenuation *= min(1.0 / p, 3.0); 
    //     }
    // }
}